import * as CanvasOv from "CanvasOverlayModule.js";

var mapColor: Microsoft.Maps.Map[] = null;
...
function LiberarMapaCalor(Posicion: number) {
    if (Posicion >= 0 && Posicion < mapColor.length) {
        mapColor[Posicion] = null;
    }
}

function TextoHexa(Valor: number) {
...
function ColorRelleno(Posicion: number, Total: number, Red: number, Green: number, Blue: number) {
...
function DibujarPoligonoCanvas(Contexto: CanvasRenderingContext2D, Puntos: Microsoft.Maps.Point[]) {
    Contexto.beginPath();
    var i;
...
function ObtenerFraccion(Val1: number, Val2: number, Buscado: number) {
...
function PuntoSobreBorde(P1: Microsoft.Maps.Point, P2: Microsoft.Maps.Point, Fraccion: number) {
    return new Microsoft.Maps.Point(P1.x + (P2.x - P1.x) * Fraccion, P1.y + (P2.y - P1.y) * Fraccion);
}

function DibujarValoresTrianguloCanvas(Contexto: CanvasRenderingContext2D,
    Punto1: Microsoft.Maps.Point, Val1: number, Punto2: Microsoft.Maps.Point, Val2: number,
    Punto3: Microsoft.Maps.Point, Val3: number, Escala1: number, Escala2: number) {
    ...
    var Puntos: Microsoft.Maps.Point[];
    ...
    var IntersV1: Microsoft.Maps.Point[] = new Microsoft.Maps.Point[2];
    var PosIntersV1: number[] = [-1, -1];
...
function DibujarValoresRectanguloCanvas(Contexto: CanvasRenderingContext2D,
    Punto1: Microsoft.Maps.Point, Val1: number, Punto2: Microsoft.Maps.Point, Val2: number,
    Punto3: Microsoft.Maps.Point, Val3: number, Punto4: Microsoft.Maps.Point, Val4: number, Escala1: number, Escala2: number) {
    ...
    var PuntoCentro: Microsoft.Maps.Point = new Microsoft.Maps.Point((Punto1.x + Punto2.x) / 2, (Punto2.y + Punto4.y) / 2);
...
function GetMapCalor(Posicion: number, NombreCanvas: string, Filas: number, Columnas: number,
    Abscisas: number[], Ordenadas: number[], Valores: number[], LatCentro: number, LngCentro: number, NivelZoom: number,
    Escala: number[], Red: number, Green: number, Blue: number) {
    ...
    var Ubicaciones: Microsoft.Maps.Location[];
    Ubicaciones = new Array<Microsoft.Maps.Location>(ValoresNecesarios);
    var i: number;
    for (i = 0; i < ValoresNecesarios; i++) {
        Ubicaciones[i] = new Microsoft.Maps.Location(Ordenadas[i], Abscisas[i]);
    }

    var Puntos: Microsoft.Maps.Point[] = <Microsoft.Maps.Point[]>mapColor[Posicion].tryLocationToPixel(Ubicaciones, Microsoft.Maps.PixelReference.control);
            ...
            for (PosColor = 0; PosColor < (Escala.length-1); PosColor++) {
                ...
                for (Fila = 0; Fila < Filas; Fila++) {
                    for (Columna = 0; Columna < Columnas; Columna++) {
                        ...
                        DibujarValoresRectanguloCanvas(ctx, Puntos[Pos0], Valores[Pos0],
                            Puntos[Pos0 + 1], Valores[Pos0 + 1],
                            Puntos[Pos2], Valores[Pos2],
                            Puntos[Pos2 + 1], Valores[Pos2 + 1],
                            Escala[PosColor], Escala[PosColor + 1]);
                    }
                }
            }
